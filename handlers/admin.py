...
def _admin_back_cancel_kb():
    ...
    try:
        kb.row(
            ...
            types.InlineKeyboardButton("✖️ إلغاء", callback_data="adm_flow:cancel"),
        )
    except Exception:
        pass
    return kb
...
# [duplicate removed] def _collect_all_user_ids() -> set[int]:
# [duplicate removed]     """
# [duplicate removed]     يرجع مجموعة بكل user_id المعروفين (من الجدول + الأدمن).
# [duplicate removed]     """
# [duplicate removed]     ids: set[int] = set()
# [duplicate removed] 
    # نسحب كل المستخدمين من الجدول
# [duplicate removed]     try:
# [duplicate removed]         rs = get_table(USERS_TABLE).select("user_id").execute()
# [duplicate removed]         rows = rs.data or []
# [duplicate removed]     except Exception:
# [duplicate removed]         rows = []
# [duplicate removed] 
# [duplicate removed]     for r in rows:
# [duplicate removed]         try:
# [duplicate removed]             uid = int(r.get("user_id") or 0)
# [duplicate removed]             if uid:
# [duplicate removed]                 ids.add(uid)
# [duplicate removed]         except Exception:
# [duplicate removed]             pass
# [duplicate removed] 
    # اختياري: إضافة الأدمن الرئيسي وباقي الأدمنين لسهولة الاختبار
# [duplicate removed]     try:
# [duplicate removed]         ids.add(int(ADMIN_MAIN_ID))
# [duplicate removed]     except Exception:
# [duplicate removed]         pass
# [duplicate removed] 
# [duplicate removed]     try:
# [duplicate removed]         for aid in ADMINS:
# [duplicate removed]             try:
# [duplicate removed]                 ids.add(int(aid))
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]     except Exception:
# [duplicate removed]         pass
# [duplicate removed] 
# [duplicate removed]     return ids
    
# [duplicate removed] def _register_admin_roles(bot):
# [duplicate removed]     @bot.message_handler(func=lambda m: m.text == "👥 صلاحيات الأدمن" and _is_admin_msg(m))
# [duplicate removed]     def admins_roles(m):
        # انتبه: لا تستورد داخل الدالة إذا المتغيرات متاحة أصلاً بالموديول
# [duplicate removed]         ids_str = ", ".join(str(x) for x in ADMINS)
# [duplicate removed]         bot.send_message(m.chat.id, f"الأدمن الرئيسي: {ADMIN_MAIN_ID}\nالأدمنون: {ids_str}")
# [duplicate removed] 
# [duplicate removed]     @bot.message_handler(func=lambda m: m.text == "⚙️ النظام" and _is_admin_msg(m))
# [duplicate removed]     @bot.message_handler(func=lambda m: (m.from_user and hasattr(m, 'text') and isinstance(m.text, str) and (m.from_user.id in ADMINS)) and _match_admin_alias(m.text, ["النظام","إعدادات النظام","اعدادات النظام","الاعدادات"]))
# [duplicate removed]     def system_menu_alias(m):
# [duplicate removed]         return system_menu(m)
# [duplicate removed]     def system_menu(m):
# [duplicate removed]         kb = types.InlineKeyboardMarkup(row_width=2)
# [duplicate removed]         kb.add(
# [duplicate removed]             types.InlineKeyboardButton("🧱 وضع الصيانة: تشغيل", callback_data="sys:maint_on"),
# [duplicate removed]             types.InlineKeyboardButton("🧱 وضع الصيانة: إيقاف",  callback_data="sys:maint_off"),
# [duplicate removed]         )
# [duplicate removed]         kb.add(
# [duplicate removed]             types.InlineKeyboardButton("🧪 فحص الصحة", callback_data="sys:health"),
# [duplicate removed]             types.InlineKeyboardButton("🧹 تنظيف الأقفال/الطوابير", callback_data="sys:cleanup"),
# [duplicate removed]         )
# [duplicate removed]         kb.add(
# [duplicate removed]             types.InlineKeyboardButton("🔁 إعادة فحص الإشتراك الإجباري", callback_data="sys:forcesub"),
# [duplicate removed]             types.InlineKeyboardButton("📜 آخر السجلات", callback_data="sys:logs"),
# [duplicate removed]         )
# [duplicate removed]         kb.add(types.InlineKeyboardButton("⬅️ رجوع", callback_data="admin:home"))
# [duplicate removed]         bot.send_message(m.chat.id, "قائمة النظام:", reply_markup=kb)
# [duplicate removed]         
# [duplicate removed]     @bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("sys:"))
# [duplicate removed]     def system_actions(c):
# [duplicate removed]         try:
# [duplicate removed]             act = c.data.split(":",1)[1]
# [duplicate removed]             if act == "maint_on":
# [duplicate removed]                 set_maintenance(True);  bot.answer_callback_query(c.id, "تم تفعيل الصيانة.")
# [duplicate removed]             elif act == "maint_off":
# [duplicate removed]                 set_maintenance(False); bot.answer_callback_query(c.id, "تم إلغاء الصيانة.")
# [duplicate removed]             elif act == "health":
# [duplicate removed]                 try:
# [duplicate removed]                     _ = get_table("features").select("id").limit(1).execute()
# [duplicate removed]                     msg = "✅ كل شيء سليم"
# [duplicate removed]                 except Exception:
# [duplicate removed]                     msg = "❌ مشكلة في الاتصال بقاعدة البيانات"
# [duplicate removed]                 bot.answer_callback_query(c.id, msg, show_alert=True)
# [duplicate removed] 
# [duplicate removed]             elif act == "cleanup":
# [duplicate removed]                 try:
# [duplicate removed]                     purge_state()           # من services.state_service (مستوردة أعلى الملف)
# [duplicate removed]                     delete_inactive_users() # من services.cleanup_service (مستوردة أعلى الملف)
# [duplicate removed]                     bot.answer_callback_query(c.id, "تم تنظيف الحالات المؤقتة.")
# [duplicate removed]                 except Exception:
# [duplicate removed]                     bot.answer_callback_query(c.id, "تعذّر التنظيف.")
# [duplicate removed] 
# [duplicate removed]             elif act == "forcesub":
# [duplicate removed]                 try:
# [duplicate removed]                     force_sub_recheck(); bot.answer_callback_query(c.id, "تمت إعادة فحص الاشتراك.")
# [duplicate removed]                 except Exception:
# [duplicate removed]                     bot.answer_callback_query(c.id, "تعذّر إعادة الفحص.")
# [duplicate removed]             elif act == "logs":
# [duplicate removed]                 tail = (get_logs_tail(900) or "")[:3500]
# [duplicate removed]                 bot.send_message(c.message.chat.id, f"آخر السجلات:\n<code>{tail}</code>", parse_mode="HTML")
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]         except Exception as e:
# [duplicate removed]             logging.exception("[ADMIN] system action failed: %s", e)
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id, "تعذّر التنفيذ")
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed] 
    # =========================
    ...
    # نفترض أن ADMINS, ADMIN_MAIN_ID, parse_user_id, USERS_TABLE, get_table معرفة فوق
# [duplicate removed] 
# [duplicate removed]     def _is_admin(uid: int) -> bool:
# [duplicate removed]         return (uid in ADMINS) or (uid == ADMIN_MAIN_ID)
# [duplicate removed] 
# [duplicate removed]     @bot.message_handler(func=lambda m: m.text == "🎟️ أكواد خصم" and _is_admin(m.from_user.id))
# [duplicate removed]     @bot.message_handler(func=lambda m: (m.from_user and hasattr(m, 'text') and isinstance(m.text, str) and _is_admin(m.from_user.id)) and _match_admin_alias(m.text, ["خصم","كود خصم","أكواد خصم","أكواد الخصم","نسب خصم"]))
# [duplicate removed]     def discount_menu_alias(m):
# [duplicate removed]         return discount_menu(m)
# [duplicate removed] 
# [duplicate removed]     def discount_menu(m):
# [duplicate removed]         kb = types.InlineKeyboardMarkup(row_width=2)
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("➕ خصم عام 1٪", callback_data="disc:new:global:1"),
# [duplicate removed]             types.InlineKeyboardButton("➕ خصم عام 2٪", callback_data="disc:new:global:2"),
# [duplicate removed]         )
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("➕ خصم عام 3٪", callback_data="disc:new:global:3"),
# [duplicate removed]             types.InlineKeyboardButton("➕ خصم لعميل",   callback_data="disc:new_user"),
# [duplicate removed]         )
# [duplicate removed]         try:
# [duplicate removed]             rows = list_discounts(limit=25) or []
# [duplicate removed]         except Exception:
# [duplicate removed]             rows = []
# [duplicate removed]         for r in rows:
# [duplicate removed]             did    = str(r.get("id"))
# [duplicate removed]             pct    = int(r.get("percent") or 0)
# [duplicate removed]             scope  = (r.get("scope") or "global").lower()
# [duplicate removed]             effective = bool(r.get("effective_active", r.get("active")))
# [duplicate removed]             ended     = bool(r.get("ends_at")) and not effective
# [duplicate removed]             state     = "🟢" if effective else ("⏳" if ended else "🔴")
# [duplicate removed]             to        = '0' if effective else '1'
# [duplicate removed] 
            # عنوان الزر
# [duplicate removed]             if scope == "user" and r.get("user_id"):
# [duplicate removed]                 title = f"{pct}٪ — عميل {r['user_id']}"
# [duplicate removed]             else:
# [duplicate removed]                 title = f"{pct}٪ — عام"
# [duplicate removed] 
# [duplicate removed]             kb.add(types.InlineKeyboardButton(f"{state} {title}",
# [duplicate removed]                                               callback_data=f"disc:toggle:{did}:{to}"))
# [duplicate removed]             kb.row(
# [duplicate removed]                 types.InlineKeyboardButton("⏳ انهاء الآن", callback_data=f"disc:end:{did}"),
# [duplicate removed]                 types.InlineKeyboardButton("🗑 حذف",        callback_data=f"disc:delete:{did}"),
# [duplicate removed]             )
# [duplicate removed] 
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("🟢 تشغيل جميع الأكواد", callback_data="disc:all:1"),
# [duplicate removed]             types.InlineKeyboardButton("🔴 إيقاف جميع الأكواد", callback_data="disc:all:0"),
# [duplicate removed]         )
# [duplicate removed]         kb.add(types.InlineKeyboardButton("📊 إحصاءات الاستخدام", callback_data="disc:stats"))
# [duplicate removed]         kb.add(types.InlineKeyboardButton("⬅️ رجوع", callback_data="admin:home"))
# [duplicate removed]         bot.send_message(m.chat.id, "لوحة الخصومات:", reply_markup=kb)
# [duplicate removed] 
# [duplicate removed] 
# [duplicate removed]     @bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("disc:"))
# [duplicate removed]     def discounts_actions(c):
# [duplicate removed]         if not _is_admin(c.from_user.id):
# [duplicate removed]             return bot.answer_callback_query(c.id, "غير مصرح.")
# [duplicate removed]         parts = (c.data or "").split(":")
# [duplicate removed]         act = parts[1] if len(parts) > 1 else None
# [duplicate removed]         if not act:
# [duplicate removed]             return bot.answer_callback_query(c.id)
# [duplicate removed] 
# [duplicate removed]         if act == "new":
# [duplicate removed]             if len(parts) < 4:
# [duplicate removed]                 return bot.answer_callback_query(c.id, "صيغة غير صحيحة.")
# [duplicate removed]             _, _, scope, pct = parts[:4]
# [duplicate removed]             try:
# [duplicate removed]                 create_discount(scope=scope, percent=int(pct))
# [duplicate removed]                 bot.answer_callback_query(c.id, "✅ تم إنشاء الخصم.")
# [duplicate removed]             except Exception as e:
# [duplicate removed]                 bot.answer_callback_query(c.id, f"❌ فشل الإنشاء: {e}")
# [duplicate removed]             return discount_menu(c.message)
# [duplicate removed] 
# [duplicate removed]         elif act == "new_user":
# [duplicate removed]             _disc_new_user_state[c.from_user.id] = {"step": "ask_user"}
# [duplicate removed]             kb = types.InlineKeyboardMarkup(row_width=2)
# [duplicate removed]             kb.row(types.InlineKeyboardButton("⬅️ رجوع", callback_data="admin:home"),
# [duplicate removed]                    types.InlineKeyboardButton("✖️ إلغاء", callback_data="disc:cancel"))
# [duplicate removed]             bot.answer_callback_query(c.id)
# [duplicate removed]             return bot.send_message(
# [duplicate removed]                 c.message.chat.id,
# [duplicate removed]                 "أرسل آيدي العميل (أرقام فقط):\nيمكنك كتابة /cancel للإلغاء.",
# [duplicate removed]                 reply_markup=kb
# [duplicate removed]             )
# [duplicate removed] 
# [duplicate removed]         elif act == "toggle":
# [duplicate removed]             if len(parts) < 4:
# [duplicate removed]                 return bot.answer_callback_query(c.id, "صيغة غير صحيحة.")
# [duplicate removed]             _, _, did, to = parts[:4]
# [duplicate removed]             try:
# [duplicate removed]                 set_discount_active(did, bool(int(to)))
# [duplicate removed]                 bot.answer_callback_query(c.id, "تم التبديل.")
# [duplicate removed]             except Exception:
# [duplicate removed]                 bot.answer_callback_query(c.id, "تعذّر التبديل.")
# [duplicate removed]             return discount_menu(c.message)
# [duplicate removed] 
# [duplicate removed]         elif act == "all":
# [duplicate removed]             to = bool(int(parts[2])) if len(parts) > 2 else False
# [duplicate removed]             try:
# [duplicate removed]                 n = _disc_toggle_all(to)
# [duplicate removed]                 bot.answer_callback_query(c.id, f"تم تحديث {n} كود.")
# [duplicate removed]             except Exception:
# [duplicate removed]                 bot.answer_callback_query(c.id, "تعذّر التحديث.")
# [duplicate removed]             return discount_menu(c.message)
# [duplicate removed] 
# [duplicate removed]         elif act == "end":
# [duplicate removed]             if len(parts) < 3:
# [duplicate removed]                 return bot.answer_callback_query(c.id, "صيغة غير صحيحة.")
# [duplicate removed]             did = parts[2]
# [duplicate removed]             try:
# [duplicate removed]                 end_discount_now(did)
# [duplicate removed]                 bot.answer_callback_query(c.id, "⏳ تم إنهاء الخصم.")
# [duplicate removed]             except Exception:
# [duplicate removed]                 bot.answer_callback_query(c.id, "تعذّر الإنهاء.")
# [duplicate removed]             return discount_menu(c.message)
# [duplicate removed]             
# [duplicate removed]         elif act == "delete":
# [duplicate removed]             if len(parts) < 3:
# [duplicate removed]                 return bot.answer_callback_query(c.id, "صيغة غير صحيحة.")
# [duplicate removed]             did = parts[2]
# [duplicate removed]             try:
# [duplicate removed]                 delete_discount(did)
# [duplicate removed]                 bot.answer_callback_query(c.id, "🗑 تم الحذف.")
# [duplicate removed]             except Exception:
# [duplicate removed]                 bot.answer_callback_query(c.id, "تعذّر الحذف.")
# [duplicate removed]             return discount_menu(c.message)
# [duplicate removed] 
# [duplicate removed]         elif act == "stats":
# [duplicate removed]             try:
# [duplicate removed]                 stats = discount_stats()
# [duplicate removed]                 text = "📊 إحصاءات الخصومات (آخر 30 يوم):\n" + "\n".join(stats or ["لا يوجد"])
# [duplicate removed]             except Exception:
# [duplicate removed]                 text = "لا تتوفر إحصاءات."
# [duplicate removed]             bot.answer_callback_query(c.id)
# [duplicate removed]             return bot.send_message(c.message.chat.id, text)
# [duplicate removed] 
# [duplicate removed]     @bot.callback_query_handler(func=lambda c: c.data == "disc:cancel")
# [duplicate removed]     def disc_cancel_cb(c):
# [duplicate removed]         _disc_new_user_state.pop(c.from_user.id, None)
# [duplicate removed]         try:
# [duplicate removed]             bot.answer_callback_query(c.id, "❎ أُلغي.")
# [duplicate removed]             pass
# [duplicate removed]         except Exception: pass
# [duplicate removed]         return discount_menu(c.message)
# [duplicate removed] 
# [duplicate removed]     @bot.message_handler(func=lambda m: _disc_new_user_state.get(m.from_user.id, {}).get("step") == "ask_user")
# [duplicate removed]     def disc_new_user_get_id(m):
# [duplicate removed]         txt = (m.text or "").strip()
# [duplicate removed]         if txt == "/cancel":
# [duplicate removed]             _disc_new_user_state.pop(m.from_user.id, None)
# [duplicate removed]             return bot.reply_to(m, "✅ تم الإلغاء.")
# [duplicate removed]         if txt == "/admin":
# [duplicate removed]             _disc_new_user_state.pop(m.from_user.id, None)
# [duplicate removed]             return admin_menu(m)
# [duplicate removed]         uid = None
# [duplicate removed]         try:
# [duplicate removed]             try:
# [duplicate removed]                 uid = parse_user_id(m.text)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]         except Exception:
# [duplicate removed]             return bot.reply_to(m, "❌ آيدي غير صالح. أعد المحاولة، أو اكتب /cancel.", reply_markup=_admin_back_cancel_kb())
# [duplicate removed]         except Exception:
# [duplicate removed]             uid = None
# [duplicate removed]         if uid is None:
# [duplicate removed]             import re
# [duplicate removed]             nums = re.findall(r"\d+", m.text or "")
# [duplicate removed]             if nums:
# [duplicate removed]                 try:
# [duplicate removed]                     uid = int("".join(nums))
# [duplicate removed]                     pass
# [duplicate removed]                 except Exception: uid = None
# [duplicate removed]         if uid is None:
# [duplicate removed]             return bot.reply_to(m, "❌ آيدي غير صالح. أعد المحاولة أو /cancel.")
# [duplicate removed]         try:
# [duplicate removed]             ex = get_table(USERS_TABLE).select("user_id").eq("user_id", uid).limit(1).execute()
# [duplicate removed]             if not (getattr(ex, "data", None) or []):
# [duplicate removed]                 return bot.reply_to(m, f"❌ الآيدي {uid} غير موجود في العملاء.")
# [duplicate removed]         except Exception:
# [duplicate removed]             return bot.reply_to(m, "❌ تعذّر التحقق من قاعدة البيانات الآن.")
# [duplicate removed] 
# [duplicate removed]         _disc_new_user_state[m.from_user.id] = {"step": "ask_pct", "user_id": uid}
# [duplicate removed]         kb = types.InlineKeyboardMarkup(row_width=3)
# [duplicate removed]         for p in (1, 2, 3):
# [duplicate removed]             kb.add(types.InlineKeyboardButton(f"{p}٪", callback_data=f"disc:new_user_pct:{uid}:{p}"))
# [duplicate removed]         kb.add(types.InlineKeyboardButton("⬅️ رجوع", callback_data="admin:home"))
# [duplicate removed]         return bot.send_message(m.chat.id, "اختر نسبة الخصم:", reply_markup=kb)
# [duplicate removed] 
# [duplicate removed]     @bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("disc:new_user_pct:"))
# [duplicate removed]     def disc_new_user_choose_pct(c):
# [duplicate removed]         if not _is_admin(c.from_user.id):
# [duplicate removed]             return bot.answer_callback_query(c.id, "غير مصرح.")
# [duplicate removed]         _, _, uid, pct = c.data.split(":", 3)
# [duplicate removed]         uid = int(uid); pct = int(pct)
# [duplicate removed]         _disc_new_user_state[c.from_user.id] = {"step": "ask_dur", "user_id": uid, "pct": pct}
# [duplicate removed]         kb = types.InlineKeyboardMarkup(row_width=2)
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("يوم",    callback_data=f"disc:new_user_dur:{uid}:{pct}:1"),
# [duplicate removed]             types.InlineKeyboardButton("3 أيام", callback_data=f"disc:new_user_dur:{uid}:{pct}:3"),
# [duplicate removed]         )
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("أسبوع",  callback_data=f"disc:new_user_dur:{uid}:{pct}:7"),
# [duplicate removed]             types.InlineKeyboardButton("♾ يدوي", callback_data=f"disc:new_user_dur:{uid}:{pct}:0"),
# [duplicate removed]         )
# [duplicate removed]         kb.add(types.InlineKeyboardButton("⬅️ رجوع", callback_data="admin:home"))
# [duplicate removed]         bot.answer_callback_query(c.id)
# [duplicate removed]         return bot.send_message(c.message.chat.id, "اختر مدة الخصم:", reply_markup=kb)
# [duplicate removed] 
    # --- Discounts: choose user duration ---
    # --- Discounts: choose user duration ---
# [duplicate removed]     @bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("disc:new_user_dur:"))
# [duplicate removed]     def disc_new_user_choose_duration(c):
# [duplicate removed]         if not _is_admin(c.from_user.id):
# [duplicate removed]             return bot.answer_callback_query(c.id, "غير مصرح.")
# [duplicate removed]         _, _, uid, pct, days = c.data.split(":", 4)
# [duplicate removed]         try:
# [duplicate removed]             uid_i  = int(uid)
# [duplicate removed]             pct_i  = int(pct)
# [duplicate removed]             days_i = int(days)
# [duplicate removed]         except Exception:
# [duplicate removed]             uid_i, pct_i, days_i = int(uid), int(pct), 0
# [duplicate removed] 
# [duplicate removed]         try:
# [duplicate removed]             create_discount(scope="user", user_id=uid_i, percent=pct_i, days=(days_i or None))
# [duplicate removed]             _disc_new_user_state.pop(c.from_user.id, None)
# [duplicate removed]             bot.answer_callback_query(c.id, "✅ تم إنشاء الخصم للمستخدم.")
# [duplicate removed] 
            # ⬅️ إشعار العميل
# [duplicate removed]             try:
# [duplicate removed]                 dur_txt = f"لمدة {days_i} يوم" if days_i > 0 else "بدون مدة محددة"
# [duplicate removed]                 msg = (
# [duplicate removed]                     f"{BAND}\n"
# [duplicate removed]                     f"🎁 تم تفعيل خصم {pct_i}% على مشترياتك {dur_txt}.\n"
# [duplicate removed]                     f"استمتع بالتوفير عند الشراء من البوت.\n"
# [duplicate removed]                     f"{BAND}"
# [duplicate removed]                 )
# [duplicate removed]                 try:
                    # لو عندك notify_user مفعّلة
# [duplicate removed]                     notify_user(bot, uid_i, _append_bot_link_for_user(msg))
# [duplicate removed]                 except Exception:
# [duplicate removed]                     bot.send_message(uid_i, _append_bot_link_for_user(msg), parse_mode="HTML")
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed] 
# [duplicate removed]         except Exception as e:
...
# [duplicate removed] 
# [duplicate removed] 
# [duplicate removed]     def _disc_toggle_all(_to: bool) -> int:
# [duplicate removed]         """تشغيل/إيقاف جميع أكواد الخصم دفعة واحدة."""
# [duplicate removed]         try:
# [duplicate removed]             items = list_discounts() or []
# [duplicate removed]         except Exception:
# [duplicate removed]             return 0
# [duplicate removed]         changed = 0
# [duplicate removed]         for it in items:
# [duplicate removed]             did = str(it.get("id"))
# [duplicate removed]             try:
# [duplicate removed]                 set_discount_active(did, bool(_to))
# [duplicate removed]                 changed += 1
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]         return changed
# [duplicate removed] 
# [duplicate removed]     def _get_user_by_id(uid: int):
# [duplicate removed]         """قراءة صف العميل من جدول houssin363 عبر user_id فقط."""
# [duplicate removed]         try:
# [duplicate removed]             r = (
# [duplicate removed]                 get_table(USERS_TABLE)
# [duplicate removed]                 .select("user_id,name,balance,admin_approved,points")
# [duplicate removed]                 .eq("user_id", uid)
# [duplicate removed]                 .limit(1)
# [duplicate removed]                 .execute()
# [duplicate removed]             )
# [duplicate removed]             rows = getattr(r, "data", None) or []
# [duplicate removed]             return rows[0] if rows else None
# [duplicate removed]         except Exception as e:
# [duplicate removed]             logging.exception("manage_user: DB error: %s", e)
# [duplicate removed]             return None
# [duplicate removed] 
# [duplicate removed]  
    # =========================
    # 👤 إدارة عميل — مبسّطة
    # =========================
# [duplicate removed] 
# [duplicate removed]     @bot.message_handler(func=lambda m: m.text == "👤 إدارة عميل" and (m.from_user.id in ADMINS or m.from_user.id == ADMIN_MAIN_ID))
# [duplicate removed]     @bot.message_handler(func=lambda m: (m.from_user and hasattr(m, 'text') and isinstance(m.text, str) and (m.from_user.id in ADMINS or m.from_user.id == ADMIN_MAIN_ID)) and _match_admin_alias(m.text, ["عميل","ادارة عميل","إدارة عميل","العميل"]))
# [duplicate removed]     def manage_user_menu(m):
# [duplicate removed]         _manage_user_state[m.from_user.id] = {"step": "ask_id"}
# [duplicate removed]         rk = types.ReplyKeyboardMarkup(resize_keyboard=True)
# [duplicate removed]         rk.row("⬅️ رجوع")
# [duplicate removed]         bot.send_message(m.chat.id, "أرسل آيدي العميل (أرقام):\n/cancel لإلغاء", reply_markup=rk)
# [duplicate removed]     @bot.message_handler(func=lambda m: _manage_user_state.get(m.from_user.id, {}).get("step") == "ask_id")
# [duplicate removed]     def manage_user_get_id(m):
# [duplicate removed]         txt = (m.text or "").strip()
# [duplicate removed]         if txt in ("/admin", "/cancel", "⬅️ رجوع"):
# [duplicate removed]             _clear_admin_states(m.from_user.id)
# [duplicate removed]             return admin_menu(m)
# [duplicate removed] 
# [duplicate removed]         try:
# [duplicate removed]             uid = parse_user_id(txt)
# [duplicate removed]         except Exception:
# [duplicate removed]             return bot.reply_to(m, "❌ آيدي غير صالح. أعد المحاولة، أو اكتب /cancel.")
# [duplicate removed] 
        # التحقق بالـ user_id فقط
# [duplicate removed]         try:
# [duplicate removed]             q = (get_table(USERS_TABLE)
# [duplicate removed]                  .select("user_id,name,balance,points")
# [duplicate removed]                  .eq("user_id", uid)
# [duplicate removed]                  .limit(1)
# [duplicate removed]                  .execute())
# [duplicate removed]             rows = getattr(q, "data", None) or []
# [duplicate removed]             row = rows[0] if rows else None
# [duplicate removed]             if not row:
# [duplicate removed]                 return bot.reply_to(m, f"❌ الآيدي {uid} غير موجود في جدول {USERS_TABLE}.")
# [duplicate removed]         except Exception as e:
# [duplicate removed]             import logging; logging.exception("manage_user: DB error: %s", e)
# [duplicate removed]             return bot.reply_to(m, "❌ تعذّر الوصول لقاعدة البيانات.")
# [duplicate removed] 
# [duplicate removed]         _manage_user_state[m.from_user.id] = {"step": "actions", "user_id": uid}
# [duplicate removed]         kb = types.InlineKeyboardMarkup(row_width=2)
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("👁️ عرض مختصر", callback_data=f"mu:profile:{uid}"),
# [duplicate removed]             types.InlineKeyboardButton("✉️ رسالة",      callback_data=f"mu:message:{uid}"),
# [duplicate removed]         )
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("⛔ حظر",        callback_data=f"mu:ban:{uid}"),
# [duplicate removed]             types.InlineKeyboardButton("✅ فكّ الحظر",  callback_data=f"mu:unban:{uid}"),
# [duplicate removed]         )
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("💸 تعويض",              callback_data=f"mu:refund:{uid}"),
# [duplicate removed]             types.InlineKeyboardButton("٪ خصم لهذا العميل",     callback_data=f"mu:disc:{uid}"),
# [duplicate removed]         )
# [duplicate removed]         kb.row(
# [duplicate removed]             types.InlineKeyboardButton("🧾 آخر 5 طلبات",        callback_data=f"mu:last5:{uid}"),
# [duplicate removed]         )
# [duplicate removed] 
# [duplicate removed]         kb.add(types.InlineKeyboardButton("⬅️ رجوع", callback_data=f"mu:back:{uid}"))
# [duplicate removed]         bot.send_message(m.chat.id, f"تم تحديد العميل <code>{uid}</code>:", parse_mode="HTML", reply_markup=kb)
# [duplicate removed] 
# [duplicate removed]     @bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("mu:"))
# [duplicate removed]     def manage_user_actions(c):
# [duplicate removed]         try:
# [duplicate removed]             _, act, uid = c.data.split(":", 2)
# [duplicate removed]             uid = int(uid)
# [duplicate removed]         except Exception:
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id, "❌ صيغة غير صحيحة.")
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]             return
# [duplicate removed] 
# [duplicate removed]         if act == "back":
# [duplicate removed]             _manage_user_state.pop(c.from_user.id, None)
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]             return admin_menu(c.message)
# [duplicate removed]         if act == "disc":
            # فتح فلو الخصم الجاهز لكن لعميل معيّن
# [duplicate removed]             _disc_new_user_state[c.from_user.id] = {"step": "ask_pct", "user_id": uid}
# [duplicate removed]             kb = types.InlineKeyboardMarkup(row_width=3)
# [duplicate removed]             for p in (1, 2, 3):
# [duplicate removed]                 kb.add(types.InlineKeyboardButton(f"{p}٪", callback_data=f"disc:new_user_pct:{uid}:{p}"))
# [duplicate removed]             kb.add(types.InlineKeyboardButton("⬅️ رجوع", callback_data="admin:home"))
# [duplicate removed]             try:
# [duplicate removed]                 bot.send_message(
# [duplicate removed]                     c.message.chat.id,
# [duplicate removed]                     f"اختر نسبة الخصم للعميل <code>{uid}</code>:",
# [duplicate removed]                     parse_mode="HTML",
# [duplicate removed]                     reply_markup=kb
# [duplicate removed]                 )
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
...
# [duplicate removed]             return
# [duplicate removed] 
# [duplicate removed]         if act == "last5":
# [duplicate removed]             try:
# [duplicate removed]                 r = get_table("purchases").select(
# [duplicate removed]                     "created_at, product_name, price"
# [duplicate removed]                 ).eq("user_id", uid).order("created_at", desc=True).limit(5).execute()
# [duplicate removed]                 rows = getattr(r, "data", []) or []
# [duplicate removed]                 lines = ["🧾 آخر 5 عمليات:"] + [
# [duplicate removed]                     f"- {str(x.get('created_at',''))[:16]} — {x.get('product_name','')} — {int(x.get('price',0)):,} ل.س"
# [duplicate removed]                     for x in rows
# [duplicate removed]                 ]
# [duplicate removed]                 bot.send_message(c.message.chat.id, "\n".join(lines))
# [duplicate removed]             except Exception:
# [duplicate removed]                 bot.send_message(c.message.chat.id, "لا يمكن جلب السجل.")
# [duplicate removed]          
            # اطلب الآيدي من جديد...
# [duplicate removed]             _manage_user_state[c.from_user.id] = {"step": "ask_id"}
# [duplicate removed]             try:
# [duplicate removed]                 rk = types.ReplyKeyboardMarkup(resize_keyboard=True)
# [duplicate removed]                 rk.row("⬅️ رجوع")
# [duplicate removed]                 bot.send_message(c.message.chat.id, "أرسل آيدي العميل من جديد:", reply_markup=rk)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
...
# [duplicate removed]             return
# [duplicate removed] 
# [duplicate removed]         if act == "message":
# [duplicate removed]             _msg_by_id_pending[c.from_user.id] = {"step": "ask_text", "user_id": uid}
# [duplicate removed]             bot.send_message(c.message.chat.id, f"اكتب الرسالة للعميل <code>{uid}</code>:", parse_mode="HTML")
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]             return
# [duplicate removed] 
# [duplicate removed]         if act == "refund":
            # أوقف ask_id مؤقتًا كي لا يتداخل مع إدخال مبلغ التعويض
# [duplicate removed]             _manage_user_state.pop(c.from_user.id, None)
# [duplicate removed]             _refund_state[c.from_user.id] = {"user_id": uid}
# [duplicate removed] 
# [duplicate removed]             bot.send_message(c.message.chat.id, "اكتب قيمة التعويض (ل.س).")
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
# [duplicate removed]               pass
...
# [duplicate removed] 
# [duplicate removed] 
# [duplicate removed]         if act == "profile":
# [duplicate removed]             try:
# [duplicate removed]                 u = get_table(USERS_TABLE).select("user_id,name,balance,points").eq("user_id", uid).limit(1).execute()
# [duplicate removed]                 row = (getattr(u, "data", None) or [None])[0] or {}
# [duplicate removed]             except Exception:
# [duplicate removed]                 row = {}
            # الرصيد من خدمة المحفظة إن متاحة
# [duplicate removed]             try:
# [duplicate removed]                 bal = get_balance(uid)
# [duplicate removed]             except Exception:
# [duplicate removed]                 bal = row.get("balance")
# [duplicate removed]             txt = (
# [duplicate removed]                 f"👤 العميل: {uid}\n"
# [duplicate removed]                 f"الاسم: {row.get('name') or '—'}\n"
# [duplicate removed]                 f"الرصيد: {('—' if bal is None else f'{int(bal):,} ل.س')}\n"
# [duplicate removed]                 f"النقاط: {int(row.get('points') or 0)}"
# [duplicate removed]             )
# [duplicate removed]             bot.send_message(c.message.chat.id, txt)
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]             return
# [duplicate removed] 
# [duplicate removed]         if act == "ban":
            # إعادة استخدام فلو الحظر العام
# [duplicate removed]             _ban_pending[c.from_user.id] = {"step": "ask_duration", "user_id": uid}
# [duplicate removed]             kb = types.InlineKeyboardMarkup(row_width=2)
# [duplicate removed]             kb.row(
# [duplicate removed]                 types.InlineKeyboardButton("🕒 1 يوم", callback_data=f"adm_ban_dur:1d"),
# [duplicate removed]                 types.InlineKeyboardButton("🗓️ 7 أيام", callback_data=f"adm_ban_dur:7d"),
# [duplicate removed]             )
# [duplicate removed]             kb.row(types.InlineKeyboardButton("🚫 دائم", callback_data="adm_ban_dur:perm"))
# [duplicate removed]             bot.send_message(c.message.chat.id, f"اختر مدة الحظر للعميل <code>{uid}</code>:", parse_mode="HTML", reply_markup=kb)
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed]             return
# [duplicate removed] 
# [duplicate removed]         if act == "unban":
# [duplicate removed]             try:
# [duplicate removed]                 unban_user(uid, c.from_user.id)
# [duplicate removed]                 log_action(c.from_user.id, "user:unban", reason=f"uid:{uid}")
# [duplicate removed]                 bot.send_message(c.message.chat.id, "✅ تم فكّ الحظر.")
# [duplicate removed]             except Exception as e:
# [duplicate removed]                 bot.send_message(c.message.chat.id, f"❌ تعذّر فكّ الحظر: {e}")
# [duplicate removed]             try:
# [duplicate removed]                 bot.answer_callback_query(c.id)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
...
# [duplicate removed] 
        # فرع افتراضي لأي فعل غير معروف
# [duplicate removed]         try:
# [duplicate removed]             bot.answer_callback_query(c.id, "❌ غير مفهوم")
# [duplicate removed]         except Exception:
# [duplicate removed]             pass
# [duplicate removed]             
# [duplicate removed]     @bot.message_handler(func=lambda m: m.from_user.id in _refund_state)
# [duplicate removed]     def _refund_amount(m):
# [duplicate removed]         st = _refund_state.get(m.from_user.id)
# [duplicate removed]         if not st:
# [duplicate removed]             return
# [duplicate removed]         uid = st["user_id"]
# [duplicate removed]         try:
# [duplicate removed]             amount = int((m.text or "").strip())
# [duplicate removed]         except Exception:
# [duplicate removed]             return bot.reply_to(m, "❌ أدخل رقم صحيح.")
# [duplicate removed] 
# [duplicate removed]         try:
# [duplicate removed]             add_balance(uid, int(amount), "تعويض إداري")
# [duplicate removed]             bot.reply_to(m, f"✅ تم تعويض <code>{uid}</code> بمقدار {amount:,} ل.س", parse_mode="HTML")
# [duplicate removed] 
            # إشعار العميل (اختياري)
# [duplicate removed]             try:
# [duplicate removed]                 note = (
# [duplicate removed]                     f"{BAND}\n"
# [duplicate removed]                     f"💸 تم إضافة تعويض إلى محفظتك بقيمة {_fmt_syp(amount)}.\n"
# [duplicate removed]                     f"لو عندك أي استفسار راسلنا.\n"
# [duplicate removed]                     f"{BAND}"
# [duplicate removed]                 )
# [duplicate removed]                 bot.send_message(uid, _append_bot_link_for_user(note), parse_mode="HTML")
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
# [duplicate removed] 
# [duplicate removed]         except Exception as e:
# [duplicate removed]             bot.reply_to(m, f"❌ فشل التعويض: {e}")
# [duplicate removed]         finally:
# [duplicate removed]             _refund_state.pop(m.from_user.id, None)
# [duplicate removed]             _manage_user_state[m.from_user.id] = {"step": "ask_id"}
# [duplicate removed]             rk = types.ReplyKeyboardMarkup(resize_keyboard=True)
# [duplicate removed]             rk.row("⬅️ رجوع")
# [duplicate removed]             try:
# [duplicate removed]                 bot.send_message(m.chat.id, "أرسل آيدي العميل من جديد:", reply_markup=rk)
# [duplicate removed]             except Exception:
# [duplicate removed]                 pass
