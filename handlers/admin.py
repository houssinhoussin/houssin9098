...
def admin_menu(msg):
    ...
    if is_primary:
        ...
        kb.row("👥 صلاحيات الأدمن", "📢 عرض اليوم")
        kb.row("📝 رسالة من عندي", "📊 استفتاء سريع")
        kb.row("📬 ترحيب — نحن شغالين", "🔄 مزامنة المنتجات (DB)")
        kb.row("⬅️ رجوع")
    else:
        kb.row("🧩 تشغيل/إيقاف المزايا", "⏳ طابور الانتظار")
        kb.row("🛒 إدارة المنتجات")
...
def register(bot, history):
    ...
    # ===== قائمة الأدمن =====
    @bot.message_handler(commands=['admin'], func=lambda m: False)
    def __admin_cmd(m):
        _clear_admin_states(m.from_user.id)
...
def _register_admin_roles(*args, **kwargs):
    """Forwarder to the primary `_register_admin_roles` to avoid duplicate definitions."""
    try:
        return globals().get("_register_admin_roles", None)(*args, **kwargs)
    except Exception:
        return None


# === Auto-generated fallback for admin callbacks (non-destructive) ===
try:
    _MISSING_ADMIN_CALLBACKS = [
  "adm_ban:cancel",
  "adm_ban:confirm",
  "adm_ban_dur:perm",
  "adm_feat_home:flat",
  "adm_feat_home:groups",
  "adm_feat_sync",
  "adm_flow:cancel",
  "adm_msgid:cancel",
  "adm_prod_back",
  ...
  "sys:maint_on"
    ...
    _MISSING_ADMIN_CALLBACKS = ["adm_ban:cancel", "adm_ban:confirm", "adm_ban_dur:perm", "adm_feat_home:flat", "adm_feat_home:groups", "adm_feat_sync", "adm_flow:cancel", "adm_msgid:cancel", "adm_prod_back", "adm_unban:cancel", "adm_unban:confirm", "admin:home", "bd_cancel", "bd_confirm", "bd_dest_channel", "bd_dest_clients", "bf_cancel", "bf_confirm", "bf_dest_channel", "bf_dest_clients", "bp_cancel", "bp_confirm", "bp_dest_channel", "bp_dest_clients", "bw_cancel", "bw_confirm", "bw_dest_channel", "bw_dest_clients", "disc:all:0", "disc:all:1", "disc:cancel", "disc:new:global:1", "disc:new:global:2", "disc:new:global:3", "disc:new_user", "disc:stats", "noop", "sys:cleanup", "sys:forcesub", "sys:health", "sys:logs", "sys:maint_off", "sys:maint_on"]

@bot.callback_query_handler(func=lambda call: _is_admin_cb(call) and call.data in set(_MISSING_ADMIN_CALLBACKS))
def _admin_missing_callbacks(call):
    try:
        bot.answer_callback_query(call.id)
    except Exception:
        pass
    try:
        bot.reply_to(call.message, "ℹ️ هذه العملية غير متاحة حاليًا من هذه اللوحة. تم إعادتك إلى لوحة التحكم.")
    except Exception:
        pass
    try:
        return admin_menu(call.message)
    except Exception:
        return
# === End auto-generated fallback ===
