...
from services.wallet_service import (
    ...
    deduct_balance,
    add_balance,                # استخدمناه لردّ الحجز قبل add_purchase
...
def register_companies_transfer(bot, history):
    ...
    def open_companies_menu(msg):
        ...
        bot.send_message(
            msg.chat.id,
            "💸 اختر الشركة التي تريد التحويل عبرها:",
            reply_markup=companies_transfer_menu(),
        )

    @bot.callback_query_handler(func=lambda call: call.data in {"company_alharam", "company_alfouad", "company_shakhashir"})
    def select_company(call):
        user_id = call.from_user.id

        # منع ازدواج الطلبات المعلقة
    ...
    def company_transfer_confirm(call):
        ...
        # — حجز مؤقت (خصم أولي) — سيُدار عند القبول/الرفض في لوحة الأدمن
        ...
        add_pending_request(
            ...
            payload={
                ...
                "total": total,
                "reserved": total,  # مبلغ محجوز لحين قرار الإدارة
            },
        )

        bot.edit_message_text(
            "✅ تم إرسال طلبك إلى الإدارة (الطابور). سيتم التنفيذ بعد الموافقة.",
            call.message.chat.id,
            call.message.message_id,
        )
        bot.send_message(
    ...
    # ====== هاندلرات قبول/رفض خاصة (تبقى للتوافق، لكنها الآن تمنع الخصم المزدوج) ======
    ...
    def admin_accept_company_transfer(call):
        try:
            ...
            reserved = int(payload.get("reserved") or total_from_btn or 0)
            company = payload.get("company")
            beneficiary_name = payload.get("beneficiary_name")
            beneficiary_number = payload.get("beneficiary_number")
            amount = int(payload.get("amount") or 0)

            # خطوة الأمان: تأكد أن لديه على الأقل «الرصيد بعد الحجز» (قد يكون 0)
            # في وضعك الحالي نحن خصمنا الحجز مسبقًا، لذا نعيده ثم نخصم عبر add_purchase
            add_balance(user_id, reserved, "إلغاء الحجز قبل التنفيذ")
            add_purchase(
                user_id=user_id,
                product_id=reserved,
                product_name=f"حوالة مالية عبر {company}",
                price=reserved,                 # add_purchase تخصم داخليًا
                player_id=beneficiary_number,
            )

            logging.info(f"[COMPANY][ADMIN][{user_id}] قبول الحوالة، الإجمالي: {reserved}")
            bot.send_message(
                user_id,
            ...
            # رسالة/صورة من الأدمن للعميل بعد التنفيذ
            ...
            logging.error(f"[COMPANY][ADMIN] خطأ أثناء تأكيد الحوالة: {e}", exc_info=True)
    ...
    def admin_reject_company_transfer(call):
        try:
            ...
            # عند الرفض: ارجع الحجز المحفوظ (إن وجد)
            res = (
                get_table("pending_requests")
                .select("id, payload")
                .eq("user_id", user_id)
            ...
            if res.data:
                row = res.data[0]
                request_id = row.get("id")
                reserved = int((row.get("payload") or {}).get("reserved") or 0)
                if reserved:
                    add_balance(user_id, reserved, "استرجاع حجز بعد رفض العملية")
            ...
            logging.error(f"[COMPANY][ADMIN] خطأ في رفض الحوالة: {e}", exc_info=True)
            bot.send_message(call.message.chat.id, f"❌ حدث خطأ: {e}")
