...
def user_quiz_state(user_id: int) -> Dict[str,Any]:
    if USE_SUPABASE:
        with httpx.Client(timeout=10) as cli:
            ...
            if r.status_code == 200 and r.json():
                return r.json()[0]
            pr = {"user_id": user_id, "template_id": _first_template_id(), "stage": 1, "q_index": 0, "paid_key": ""}
...
def register_wrong_attempt(user_id: int): pass

def compute_stage_reward_syp_safe() -> float:
    eco = _eco_get()
    op = float(eco.get("op_free_balance",0.0))
    if op <= 0: return 0.0
    cap_ratio = float(load_settings()["economy"].get("op_payout_soft_cap_ratio",0.03))
    pay = max(0.0, min(op * cap_ratio, op))
    eco["op_free_balance"] = op - pay
    _eco_set(eco)
    if pay > 0: _tx({"kind":"stage_reward","amount":pay})
    return pay
...
def advance_after_correct(user_id: int) -> Tuple[str, Dict[str,Any]]:
    ...
    if st_q >= len(items):
        # stage done
        reward = compute_stage_reward_syp_safe()
        add_balance(user_id, reward)
        _tx({"kind":"stage_done","user_id":user_id,"stage":stage_no,"reward":reward})
